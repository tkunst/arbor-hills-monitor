...
- **Given** an urgent document whose same-day `[URGENT]` email actually sends (SMTP configured, message handed to the server); **When** the next Sunday digest is composed; **Then** the item is ALSO listed under an "Urgent items from earlier this week" section, labeled with the date/time its urgent alert actually went out, positioned before the routine new-this-week content — so a reader who only reads the digest still learns an urgent event happened that week, and a reader who already got the same-day email recognizes it as a repeat, not a new event. **Given** the same-day send instead fails (an exception) OR silently no-ops (SMTP unconfigured / no recipients — `send_email` returns `False` rather than raising); **Then** the item is queued to **neither** the digest nor the recap in either case (an alert that never went out has nothing to recap — matches AR-1's existing handling of a failed urgent send; the no-op case was closed 2026-08-21 after `send_email`/`send_urgent_alert` were given an explicit `bool` return so a misconfigured SMTP path can't be durably recorded as "sent"). A week with an urgent recap and zero routine documents still sends a digest (recap-only); the same-day `[URGENT]` send itself (trigger, content, recipients) is unchanged.
- `watcher.py:94-133` (`_route_urgent_or_digest`); `email_alerts.py:103-129` (`format_digest_body`'s `urgent_recap` section); `email_alerts.py:176-207` (`send_email`'s `bool` return)
- policy
...
`woi_table_parser.py` is wired into the **live** watcher/backfill via `woi_router.py` (ADR 005 "Integration (DONE)"; see Note 6), and is also invoked directly by `scripts/woi_summary.py` / `scripts/co_summary.py` for the full hand-to-EGLE dump. It exhaustively parses the ~180–320-page WOI Status Reports where the generic keyword-windowing would capture <5% of readings.
...
4. **`_classify_annual` short-circuited on exactly 0.0 years remaining — FIXED (PR #6, `e90e994`, 2026-07-12).** The guard was `if yrs and yrs < floor` (`wds_watcher.py:109`); in Python `0.0` is falsy, so an Annual Report stating **0.0 years of capacity remaining** — the single most R1-critical case (airspace exhausted) — skipped the `notable` branch and returned `watch`. It is now `if yrs < floor`, which correctly fires `notable`/R1 on `0.0` and negatives (regression test `test_annual_zero_years_is_notable`). **Design question — RESOLVED (Trisha, 2026-07-13):** Does EGLE ever report exactly `0.0` as a real reading versus a missing-data sentinel? It is unknowable, and EGLE's behavior could change over time — so alerting on `0.0` is the **intentional, permanent** choice, not provisional pending confirmation. (A blank already raises in `float()` and is caught, so only a literal `0`/`0.0` hits this path; for a monitor a false alert is cheap and a missed "airspace exhausted" signal is not.)
...
6. **Live-pipeline evidence-completeness gap on large reports — RESOLVED (`woi_router.py`, 2026-07-13).** In a >30-page report, DC-5 shows the classifier only pages containing a signal keyword plus the cover; a real measurement on a page with no keyword was never seen — and for the semi-annual WOI Status Reports that meant <5% of ~14,000 readings captured and a measured ≥145 °F exceedance could be missed entirely (no measurement emitted → `is_urgent` never fires). **`woi_router.py` now routes WOI reports to the exhaustive `woi_table_parser` in the live watcher/backfill**, replacing `parsed.measurements` with the validated set before `is_urgent`. Detection is content-based — the Attachment-1 "Gas Extraction Report" header + page count, **not** the nSITE filename (the real reports are filed as generic "nForm Document"/"Site", so a name gate would never fire). The "WOI cluster … disconnected" topology note is retired. **Confidence: High.** **Eng question — RESOLVED (2026-07-13):** yes; WOI Status Reports route automatically now. See ADR 005 "Integration (DONE)" and Note 6's sibling safety rules (a valid-reading-count data-loss guard, and peak-temp preservation so a cool report never false-fires on a permitted ceiling).
