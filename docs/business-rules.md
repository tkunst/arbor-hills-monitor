...
| MM-1 | Meeting = 2nd Wednesday of the month (computed) | MMPC scheduling | calculation |
| MM-2 | Published calendar overrides the computed rule | MMPC scheduling | policy |
| MM-3 | Poll for minutes: start +3 days, 10-day window | MMPC scheduling | policy |
| MM-4 | Minutes "posted" = HTTP 200 with >500-byte body | MMPC scheduling | validation |
| MM-5 | One minutes alert per meeting | MMPC scheduling | state-transition |
| MM-6 | MMPC archive: type = Agenda/Minutes/Other, dedup from Sheet | MMPC scheduling | policy |
...
### MM-1 — Meeting date = 2nd Wednesday of the month (computed fallback)
- **Given** any month with `meeting_weekday=2` (Wed) and `meeting_week_of_month=2`; **When** the meeting date is computed and no explicit calendar is set; **Then** it returns the 2nd Wednesday (e.g. for a month whose 1st is a Friday, the 2nd Wednesday is the 13th).
- `mmpc_watcher.py:27-40`; parameters `config.yml:114-115`
- calculation

### MM-2 — Published calendar overrides the computed rule
- **Given** `meeting_dates` lists real 2026 meetings (Aug moved to the 19th = 3rd Wed; no November meeting; Apr/May cancelled); **When** the active-polling meeting is determined; **Then** the explicit list is used and the 2nd-Wednesday computation is ignored — so no phantom reminder fires for a skipped or moved meeting.
- `mmpc_watcher.py:47-84`; parameters `config.yml:125-134`
- policy

### MM-3 — Minutes polling window: start +3 days, run 10 days
- **Given** a meeting on 2026-07-08; **When** deciding whether to poll for minutes today; **Then** today must fall between meeting+3 days (2026-07-11) and meeting+3+10 days (2026-07-21) inclusive.
- `mmpc_watcher.py:63-84`; parameters `config.yml:116-117`
- policy

### MM-4 — "Minutes posted" = HTTP 200 with a >500-byte body
- **Given** the CivicClerk minutes URL is fetched during a polling window; **When** the response is HTTP 200 and the body exceeds 500 bytes; **Then** minutes are treated as posted; the placeholder URL `VERIFY_BEFORE_DEPLOY` always returns not-found.
- `mmpc_watcher.py:87-99`
- validation

### MM-5 — One minutes alert per meeting
- **Given** a meeting whose minutes have already been flagged found; **When** the watcher polls again within the window; **Then** no second email is sent (the per-meeting `mmpc_minutes_found` flag gates it).
- `watcher.py:181-195`
- state-transition
...
| MMPC `meeting_weekday` / `meeting_week_of_month` | **2 / 2** (2nd Wed) | `config.yml:114-115` | MM-1 |
| MMPC `poll_start_days_after` / `poll_window_days` | **3 / 10** | `config.yml:116-117` | MM-3 |
| minutes-posted body floor | **500 bytes** | `mmpc_watcher.py:95` | MM-4 |
