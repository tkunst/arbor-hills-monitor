...
| A malformed `mirrors:` config entry archives nothing and still exits 0 | low — found by independent code review | none; indistinguishable from "no new files today" | fixed: a malformed entry now sets a non-zero exit, same treatment as a fetch failure (it never self-heals, unlike an unprovisioned folder secret, which is expected mid-activation) |
| `enabled: true` but no folder secret provisioned yet still creates an empty Sheet tab and refreshes the OAuth token before discovering nothing is archivable | low — found by independent code review | cosmetic (an empty tab); not harmful, just wasted work | fixed: an up-front check skips Sheets/Drive setup entirely when no mirror is provisioned — unit-tested (`test_no_mirror_provisioned_skips_sheets_and_drive_setup_entirely`) |
